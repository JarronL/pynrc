...
# def bg_sensitivity_extended(filter_or_bp, pupil=None, mask=None, module='A', pix_scale=None,
# 	sp=None, units=None, nsig=10, extended=False,
# 	tf=10.737, ngroup=2, nf=1, nd2=0, nint=1,
# 	coeff=None, fov_pix=11, oversample=4, quiet=True, **kwargs):
# 	"""
# 	Estimates the surface brightness sensitivity for a set of instrument parameters.
...
# 	Spectrum Settings
# 	-------------------
# 	sp_norm : A pysynphot spectral object to calculate sensitivity 
...
# 	t0 = time.time()
# 	data = gen_image_coeff(bp, pupil, mask, module, sp_norm, 
# 		coeff, fov_pix, oversample, **kwargs)
# 	t1 = time.time()
...
# 			snr_arr = []
# 			for jj,f in enumerate(fact_arr):
#  				#im_var = data/f/tint + var_const
# 
# 				im_var = pix_noise(ngroup=ngroup, nf=nf, nd2=nd2, tf=tf, 
# 					fzodi=fzodi_pix, fsrc=data/f, **kwargs)**2
# 					
# 				fsrc2 = np.ones([4,5]) * 0.001
# 				fsum2 = fsrc2.sum()
# 				im_var2 = pix_noise(ngroup=ngroup, nf=nf, nd2=nd2, tf=tf, 
# 					fzodi=fzodi_pix, fsrc=fsrc2, **kwargs)**2
# 				im_sig = np.sqrt(im_var2.sum() / nint)
# 				if ii==1: print('{:.5f} {:.5f} {:.2f}'.format(fsum2,im_sig,fsum2/im_sig))
# 	
# 				 # Create HDU list to pass to radial_profile()
# 				var_hdu = fits.PrimaryHDU(im_var)
# 				var_hdu.header.append()
# 				var_hdu.header.append(('PIXELSCL', pix_scale))
# 				var_hdu_list = fits.HDUList(var_hdu)
# 				_, _, EE_var = radial_profile(var_hdu_list, EE=True)
...
# 
# 		if quiet == False:
# 			print('Sensitivity ({0}-sigma): {1:.2f} {2}'.format(nsig, bglim, units))
# 
# 		return {'sensitivity':bglim, 'units':units, 'nsig':nsig, 'Spectrum':sp.name}
